...
import { getCurrentUser, getInventoryItems, getInvoices, getSuppliers } from "@/lib/api";
import {
  calculateSalesData,
  getLowStockItems,
...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    setLoading(true);
    try {
      // Fetch raw data once in parallel
      const [inventory, invoicesData, suppliersData] = await Promise.all([
        getInventoryItems(),
        getInvoices(),
        getSuppliers(),
      ]);
      ...
      // Calculate stats using pre-fetched raw data
      const [
        statsData,
        lowStock,
        sales,
        ...
        calculateTotalStats(invoicesData, inventory),
        getLowStockItems(inventory),
        calculateSalesData(invoicesData),
        calculateRevenueByDate(invoicesData),
        calculateSupplierAnalytics(suppliersData, inventory, invoicesData),
        calculateRestockPredictions(inventory, invoicesData),
      ]);
      setStats(statsData);
      ...
      setSupplierAnalytics(suppliersStats);
        ...
        {loading ? (
          ...
          <div className="space-y-8">
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-6 grid-cols-1">
                <SalesPerformance invoices={invoices} />
              </div>
            </div>
            ...
            {isManagerOrAdmin() && (
              <div className="space-y-4">
                ...
                <div className="grid gap-6 grid-cols-1">
                  <ProfitSummary invoices={invoices} items={inventoryItems} />
                </div>
              </div>
            )}
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-6 grid-cols-1 lg:grid-cols-2">
                <CustomerInsights invoices={invoices} />
                <RecentActivityFeed invoices={invoices} items={inventoryItems} />
              </div>
            </div>
          </div>
        )}
