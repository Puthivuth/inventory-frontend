      ...
      <main className="flex-1 p-4 sm:p-6 lg:p-8 min-w-0">
        ...
        {dbError === "database_not_setup" && (
          <Alert variant="destructive" className="mb-6">
            ...
            <AlertDescription>
              The inventory database table hasn't been created yet. Please run
              the SQL scripts from the sidebar to set up the database:
              <ol className="list-decimal list-inside mt-2 space-y-1">
                <li>
                  <code className="text-sm bg-muted px-1 py-0.5 rounded">
                    scripts/001_create_inventory_table.sql
                  </code>
                </li>
                <li>
                  <code className="text-sm bg-muted px-1 py-0.5 rounded">
      ...
      <div className="p-4 border-b flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Track all stock additions and purchases
          </p>
        </div>
            ...
            {records.length === 0 ? (
              <tr>
                <td
                  ...
                  No stock records found. Start adding stock to see history
                  here.
                </td>
              </tr>
            ) : (
                ...
                .map((record) => (
                  <tr key={record.newstockId} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3 text-sm font-semibold">
                      $
                      {(
                        parseFloat(record.purchasePrice) * record.quantity
                      ).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-sm">
                      {record.supplierName || "-"}
