...
export function SalesPerformance({ invoices: propInvoices }: SalesPerformanceProps = {}) {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        const weekInvoices = paidInvoices.filter(
          (inv) => new Date(inv.createdAt) >= weekAgo,
        );
        const monthInvoices = paidInvoices.filter(
          (inv) => new Date(inv.createdAt) >= monthAgo,
        );
        ...
        const avgOrderValue =
          monthInvoices.length > 0 ? monthSales / monthInvoices.length : 0;
        const growthRate =
          yesterdaySales > 0
    ...
    <div className="grid gap-4 md:grid-cols-4 col-span-full">
      ...
      <Card>
        ...
        <CardContent>
          ...
          <p className="text-xs text-muted-foreground">
            {metrics.monthOrders} orders this month
          </p>
        </CardContent>
      </Card>
    </div>
